...
//Given the starting energy levels of the dumbo octopuses in your cavern, simulate 100 steps. How many total flashes are there after 100 steps?
    ...
    public String solveDayPartOne() {
        ...
        return String.valueOf(countFlashes);
    }
    ...
    private void upgradeNeigbors(int[][] grid, int r, int c) {
        int[] delta = new int[] { -1, 0, 1 };
        for (int row = 0; row < delta.length; row++) {
            for (int col = 0; col < delta.length; col++) {
                if (row == 1 && col == 1)
                    continue;
                if (isCanFlash(grid, r + delta[row], c + delta[col]))
                    grid[r + delta[row]][c + delta[col]]++;
            }
        }
    }

    private boolean isCanFlash(int[][] grid, int row, int col) {
    ...
    private void readInput() {
        try {
            input = new BufferedReader(new FileReader("src/Ressources/inputDay11.txt")).lines().toArray(String[]::new);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
